...
struct mt7927_mcu_eeprom_info {
	__le32 addr;
	__le32 valid;
	u8 data[MT7927_EEPROM_BLOCK_SIZE];
} __packed;
...
struct mt7927_mcu_reg_event {
	__le32 reg;
	__le32 val;
} __packed;

struct mt7927_mcu_ant_id_config {
	u8 ant_id[4];
} __packed;

struct mt7927_txpwr_req {
	u8 _rsv[4];
	__le16 tag;
	__le16 len;
...
enum {
	...
	MT7927_TM_TESTMODE,
	MT7927_TM_ICAP,
	MT7927_TM_ICAP_OVERLAP,
	MT7927_TM_WIFISPECTRUM,
};

struct mt7927_rftest_cmd {
	u8 action;
	u8 rsv[3];
	__le32 param0;
	__le32 param1;
...
#define MT7927_STA_UPDATE_MAX_SIZE	(sizeof(struct sta_req_hdr) +		\
...
#define MT7927_BSS_UPDATE_MAX_SIZE	(sizeof(struct bss_req_hdr) +		\
...
struct mt7927_sku_tlv {
	u8 channel;
	s8 pwr_limit[MT_CONNAC3_SKU_POWER_LIMIT];
} __packed;

struct mt7927_tx_power_limit_tlv {
	u8 rsv[4];
...
mt7927_mcu_get_cipher(int cipher)
{
	switch (cipher) {
...
int mt7927_mcu_sched_scan_enable(struct mt76_phy *phy,
				 struct ieee80211_vif *vif,
				 bool enable);
