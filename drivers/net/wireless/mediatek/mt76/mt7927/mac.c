...
static void mt7927_mac_sta_poll(struct mt792x_dev *dev)
{
	...
	while (true) {
		...
		idx = mlink->wcid.idx;
		addr = mt7927_mac_wtbl_lmac_addr(dev, idx, MT_WTBL_AC0_CTT_OFFSET);
		...
		addr = mt7927_mac_wtbl_lmac_addr(dev, idx, 6);
		val = mt76_rr(dev, addr);
		if (rate->flags & RATE_INFO_FLAGS_EHT_MCS) {
			addr = mt7927_mac_wtbl_lmac_addr(dev, idx, 5);
			val = mt76_rr(dev, addr);
		...
		addr = mt7927_mac_wtbl_lmac_addr(dev, idx, 34);
		val = mt76_rr(dev, addr);
...
static int mt7927_reverse_frag0_hdr_trans(struct sk_buff *skb, u16 hdr_gap)
{
...
static int
mt7927_mac_fill_rx_rate(struct mt792x_dev *dev,
...
static int
mt7927_mac_fill_rx(struct mt792x_dev *dev, struct sk_buff *skb)
{
	...
	if (rxd1 & MT_RXD1_NORMAL_GROUP_3) {
		...
		ret = mt7927_mac_fill_rx_rate(dev, status, sband, rxv, &mode);
		if (ret < 0)
			return ret;
	}
	...
	if (hdr_trans && ieee80211_has_morefrags(fc)) {
		if (mt7927_reverse_frag0_hdr_trans(skb, hdr_gap))
			return -EINVAL;
		hdr_trans = false;
	} else {
		int pad_start = 0;
...
static void
mt7927_mac_write_txwi_8023(__le32 *txwi, struct sk_buff *skb,
...
static void
mt7927_mac_write_txwi_80211(struct mt76_dev *dev, __le32 *txwi,
			    struct sk_buff *skb,
...
void
mt7927_mac_write_txwi(struct mt76_dev *dev, __le32 *txwi,
		      ...
		      enum mt76_txq_id qid, u32 changed)
{
	...
	if (is_8023)
		mt7927_mac_write_txwi_8023(txwi, skb, wcid);
	else
...
EXPORT_SYMBOL_GPL(mt7927_mac_write_txwi);

static void mt7927_tx_check_aggr(struct ieee80211_sta *sta, struct sk_buff *skb,
				 struct mt76_wcid *wcid)
{
	struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
...
static bool
mt7927_mac_add_txs_skb(struct mt792x_dev *dev, struct mt76_wcid *wcid,
...
void mt7927_mac_add_txs(struct mt792x_dev *dev, void *data)
{
	...
	mt7927_mac_add_txs_skb(dev, wcid, pid, txs_data);
	if (!wcid->sta)
		goto out;
		      ...
		      struct list_head *free_list)
{
	...
	if (sta) {
		if (likely(t->skb->protocol != cpu_to_be16(ETH_P_PAE)))
			mt7927_tx_check_aggr(sta, t->skb, wcid);
...
EXPORT_SYMBOL_GPL(mt7927_txwi_free);

static void
mt7927_mac_tx_free(struct mt792x_dev *dev, void *data, int len)
{
	...
	for (cur_info = &tx_free[2]; count < total; cur_info++) {
		...
		for (i = 0; i < 2; i++) {
			...
			mt7927_txwi_free(dev, txwi, sta, wcid, &free_list);
		}
	}

	mt7927_mac_sta_poll(dev);
...
bool mt7927_rx_check(struct mt76_dev *mdev, void *data, int len)
{
	...
	case PKT_TYPE_TXRX_NOTIFY:
		...
		mt7927_mac_tx_free(dev, data, len); /* mmio */
		return false;
	case PKT_TYPE_TXS:
		for (rxd += 4; rxd + 12 <= end; rxd += 12)
			mt7927_mac_add_txs(dev, rxd);
		return false;
	default:
		return true;
	}
}
EXPORT_SYMBOL_GPL(mt7927_rx_check);
			 ...
			 struct sk_buff *skb, u32 *info)
{
	...
	case PKT_TYPE_TXRX_NOTIFY:
		/* PKT_TYPE_TXRX_NOTIFY can be received only by mmio devices */
		mt7927_mac_tx_free(dev, skb->data, skb->len);
		napi_consume_skb(skb, 1);
		break;
...
EXPORT_SYMBOL_GPL(mt7927_queue_rx_skb);

static void
mt7927_vif_connect_iter(void *priv, u8 *mac,
			struct ieee80211_vif *vif)
{
	...
	for_each_set_bit(i, &valid, IEEE80211_MLD_MAX_NUM_LINKS) {
		...
		mt7927_mcu_set_tx(dev, bss_conf);
	}
...
void mt7927_coredump_work(struct work_struct *work)
{
	...
	while (true) {
		...
		skb_pull(skb, sizeof(struct mt7927_mcu_rxd) + 8);
...
/* usb_sdio */
static void
mt7927_usb_sdio_write_txwi(struct mt792x_dev *dev, struct mt76_wcid *wcid,
			   ...
			   struct sk_buff *skb)
{
	...
	mt7927_mac_write_txwi(&dev->mt76, txwi, skb, wcid, key, pid, qid, 0);
				   ...
				   struct mt76_tx_info *tx_info)
{
	...
	mt7927_usb_sdio_write_txwi(dev, wcid, qid, sta, key, pktid, skb);
...
EXPORT_SYMBOL_GPL(mt7927_usb_sdio_tx_prepare_skb);
...
EXPORT_SYMBOL_GPL(mt7927_usb_sdio_tx_complete_skb);
...
EXPORT_SYMBOL_GPL(mt7927_usb_sdio_tx_status_data);
